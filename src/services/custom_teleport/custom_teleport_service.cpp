...
namespace big
{
	...
	{
		return g_file_manager.get_project_file("teleport_location.json").get_path();
	}

	std::vector<teleport_location> custom_teleport_service::saved_locations_filtered_list(std::string filter)
	{
		std::vector<teleport_location> filterlist{};
		...
		for (teleport_location& location : all_saved_locations | std::views::values | std::views::join)
	...
	bool custom_teleport_service::fetch_saved_locations()
	{
		...
		try
		{
			...
			nlohmann::json j;
			file >> j;
			all_saved_locations = j.get<std::map<std::string, std::vector<teleport_location>>>();

			return true;
		}
		catch (const std::exception& e)
		{
			LOG(INFO) << "Failed fetching saved locations: " << e.what() << '\n';
			return false;
		}
	...
	bool custom_teleport_service::save_new_location(const std::string& category, teleport_location t)
	{
		...
		g_notification_service.success("Custom Teleport", std::format("Succesfully saved location {}", t.name));
	...
	teleport_location* custom_teleport_service::get_saved_location_by_name(std::string name)
	{
