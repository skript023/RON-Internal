...
namespace big
{
	float draw_notification(float start_pos, ImDrawList* dl, std::string title, std::string message, ImVec4 color)
	{
		...
		dl->AddRectFilled({ (float)g_pointers->m_resolution->x - 360.f, 10.f + start_pos }, { (float)g_pointers->m_resolution->x - 10.f, start_pos + 55.f + total_size }, g_settings.window.color);
		dl->AddRectFilledMultiColor({ (float)g_pointers->m_resolution->x - 360.f, 10.f + start_pos }, { (float)g_pointers->m_resolution->x - 255.f, start_pos + 55.f + total_size }, fadeBegin, fadeEnd, fadeEnd, fadeBegin);
